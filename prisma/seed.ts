...
const itemSeeds = [
  {
    ...
    item_type: "explain",
    prompt:
      "In your own words, explain the difference between authentication and authorization for an API request.",
    correct_answer:
...
async function main() {
  ...
  const domain = await prisma.domain.create({
    data: {
      ...
      business_goal:
        "Reduce time-to-proficiency by repairing misconceptions before customer-facing technical scenarios.",
      target_performance_goal:
        "Correctly diagnose novel API auth and permission failures with calibrated confidence.",
      ...
      transfer_scenarios: [
        "Customer says integration worked yesterday but now fails for only one subset of users."
      ],
      scoring_rubrics: {
        explanation_quality:
          "0 no meaningful explanation; 5 clear, causal, transferable explanation with boundaries and counterexamples",
        transfer:
          "0 cannot apply; 5 adapts flexibly and explains limits in a novel scenario"
      },
      expert_explanations: [
  ...
  const experimental = await prisma.user.create({
    data: {
  ...
  for (const user of [admin, experimental, control]) {
    for (const conceptId of conceptByKey.values()) {
